...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedKey && storedTimestamp) {
        ...
        // Missing timestamp or key, clear just in case
        if (storedKey && !storedTimestamp) {
             localStorage.removeItem('gemini_api_key');
        }
    }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    // Strictly require API Key from user settings
    if (!apiKey) {
  ...
  const handleUpscale = useCallback(async (resolution: '2K' | '4K') => {
      // We upscale the currently visible result if exists, otherwise original
      ...
      try {
          ...
          setEditedImage(watermarkedDataUrl);
           ...
           console.error(err);
           // Handle "Requested entity was not found" which usually implies API key issues or billing
    ...
    <div className="flex flex-col h-screen bg-gray-900 text-gray-100">
      <header className="bg-gray-800 border-b border-gray-700 p-4 shadow-md">
        <div className="flex items-baseline gap-3">
            <h1 className="text-2xl font-bold tracking-wider text-white uppercase">ARCH-HANDOL</h1>
            <span className="text-sm md:text-base text-gray-400 font-medium">(주)한돌건축사사무소 최인영건축사</span>
        </div>
      </header>
      <main className="flex-grow grid grid-cols-1 md:grid-cols-[260px_1fr_320px] lg:grid-cols-[300px_1fr_360px] gap-4 p-4 overflow-hidden relative">
        <LeftPanel
            ...
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl max-w-md w-full p-6">
                    ...
                    <p className="text-gray-300 mb-4 text-sm">
                        ...
                        입력하신 키는 <strong>브라우저(Local Storage)에만 저장</strong>되며 서버로 전송되지 않습니다.
                        <br/>보안을 위해 24시간 후 자동 만료됩니다.
                    </p>
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold transition-colors"
                        >
                            저장하기
                        </button>
                        <button
