...
const CenterPanel: React.FC<CenterPanelProps> = ({
  ...
  const renderContent = () => {
    ...
    if (originalImage) {
      return (
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          <canvas
            ref={maskCanvasRef}
            className={`absolute top-0 left-0 ${activeTool === Tool.MASK ? 'cursor-crosshair' : 'cursor-default'}`}
    ...
    return (
      <div {...getRootProps()} className={`w-full h-full border-4 border-dashed rounded-xl flex items-center justify-center text-center transition-colors duration-300 ${isDragActive ? 'border-indigo-500 bg-gray-700' : 'border-gray-600 hover:border-gray-500'}`}>
        <input {...getInputProps()} />
        <div className="flex flex-col items-center text-gray-400">
            <UploadIcon className="w-16 h-16 mb-4" />
            <h3 className="text-2xl font-bold">이미지를 드래그 앤 드롭하세요</h3>
            <p className="mt-2">또는 클릭하여 파일을 선택하세요</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div ref={containerRef} className="bg-gray-800/50 rounded-lg flex flex-col items-center justify-center shadow-inner overflow-hidden relative">
